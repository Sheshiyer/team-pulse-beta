...
export class MigrationService {
  ...
  async syncEmployeeData(forceSync: boolean = false): Promise<void> {
    try {
      ...
      for (const user of clockifyUsers) {
        try {
          const existingEmployee = existingEmployees.find(emp => emp.email === user.email);
          
          // Skip if employee exists and no force sync
          if (existingEmployee && !forceSync) {
            console.log(`Skipping ${user.email} - already exists and no force sync`);
            continue;
          }
          ...
          // Find user's group
          const userGroup = groups.find(group => group.userIds.includes(user.id));
          ...
          const weeklyLogs = timeEntries.map(entry => ({
            ...
            logoutTime: new Date(entry.timeInterval.end || new Date().toISOString())
          }));
          ...
          const employee: Omit<Employee, 'id'> = {
            ...
            customDetails: existingEmails.has(user.email) 
              ? existingEmployees.find(emp => emp.email === user.email)?.customDetails
              : undefined
          };
  ...
  async startPeriodicSync(intervalMinutes: number = 30): Promise<void> {
    if (this.syncIntervalId) {
      console.warn('Periodic sync is already running. Stop it first to restart.');
      return;
    }
